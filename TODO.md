...
- [x] **SOS — manual trigger only.** Either an explicit spoken "sos" command or the button in
  ...
  **saves** the rolling instant-replay clip (last 1/5/10 min of shared desktop video+audio,
  configurable) to `Videos/KitsuNexus/` (created automatically if missing), and additionally
...
- [x] **Clips are now real, playable .mp4 files (were .webm).** The main process now stitches the
  ...
  `sos-clip-*.mp4` in `Videos/KitsuNexus/`. The same finished .mp4 (not the raw segments) is
...
- [x] **Fixed: local Whisper crashed with "ENOTDIR" and models appeared to re-download every
  ...
  `%AppData%/KitsuNexus/models/whisper/onnx-community/whisper-tiny.en/`), always writable and
...
- [~] **Discord Activity (VRChat status panel) + official shared Discord bot.** New standalone
  ...
  official `https://kitsunexus.nekosunevr.co.uk` deployment (`DEFAULT_NEKOSUNE_BACKEND_URL`
...
- [x] **Avatar favorites (official)** — `getFavoriteAvatars()` (`GET /avatars/favorites`), shown alongside friend/world favorites on the Favorites page. Fixed: items were all landing in the default group ("avatars1"/"worlds1") because those endpoints don't return a group field — group tags are now merged in from `/favorites` (matching VRCX), so all groups show correctly.
...
- [x] **VRC+ gate on avatar Local Favorites** — Local Favorites for avatars requires an active VRC+ subscription on the VRChat account (checked via the `system_supporter` tag), except the project owner's own paired device (`kitsunexus-server` `GET /api/me` exposes the paired KitsuNexus account `role`; cached client-side as `cloudSync.role`). See `canLocalFavAvatars()` in [renderer.js](renderer.js) and `isOwner()`/`refreshMe()` in `modules/favorites/cloudSync.js`.
...
- [x] **Fixed: updater failed with `EPERM` trying to download into the install directory.** It
  tried the install directory first (e.g. `Program Files\KitsuNexus\`) and only fell back to
...
- History DB: `KitsuNexus-history.sqlite` in the app's user-data folder.
