...
async function setMute (mute) {
  ...
  try {
    ...
    await member.voice.setMute(!!mute, 'KitsuNexus DiscordOSC')
    return { ok: true }
...
async function setDeaf (deaf) {
  ...
  try {
    ...
    const member = guild && guild.members.cache.get(targetUserId)
    if (!member) return { ok: false, error: 'Member not found' }
    await member.voice.setDeaf(!!deaf, 'KitsuNexus DiscordOSC')
    return { ok: true }
  } catch (err) { return { ok: false, error: err.message } }
}
...
// ---------------------------------------------------------------------------
// Official KitsuNexus bot mode: no token, no per-user bot to create. Instead
// of running our own discord.js gateway Client, poll the KitsuNexus backend
